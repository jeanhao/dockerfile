...
def test_gen_tag_from_filepath():
    ...
    assert tag == 'registry.cn-shanghai.aliyuncs.com/russellcloud/tensorflow:1.0.1-gpu-py3_aws'

    tag = gen_tag_from_filepath(
        'dl/../dl/tensorflow/1.0.1/Dockerfile-py2_aws')
    assert tag == 'registry.cn-shanghai.aliyuncs.com/russellcloud/tensorflow:1.0.1-py2_aws'

    tag = gen_tag_from_filepath(
        'dl/tensorflow/1.4.0/Dockerfile-py3.gpu.cuda9cudnn7_aws')
    assert tag == 'registry.cn-shanghai.aliyuncs.com/russellcloud/tensorflow:1.4.0-gpu.cuda9cudnn7-py3_aws'
...
def test_gen_target_env_from_tag():
    ...
    target, env = gen_target_env_from_tag('registry.cn-shanghai.aliyuncs.com/russellcloud/tensorflow:1.0.1-py2_aws')
    assert target == '1.0.1'
    assert env == 'py2_aws'

    target, env = gen_target_env_from_tag('registry.cn-shanghai.aliyuncs.com/russellcloud/tensorflow:1.0.1-gpu.cuda11cudnn10-py2_aws')
    assert target == '1.0.1'
